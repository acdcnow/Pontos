...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    TIME_ENTRIES = getattr(device_const, "TIME_ENTRIES", {})
...
class PontosTimeEntry(TimeEntity):
    ...
    def __init__(self, hass, entry, device_info, key, config):
        ...
        self._attr_unique_id = slugify(
            f"{device_identifier(device_info, entry)}_{key}_time"
        )
        self._attr_native_value = None
        self._available = True
        self._sensor_unique_id = (
            slugify(f"{device_identifier(device_info, entry)}_{self._sensor}")
            if self._sensor
            else None
        )
