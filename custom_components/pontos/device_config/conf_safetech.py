...
from .common import build_microleakage_binary_sensor
from .common import build_network_sensors
from .common import build_profile_count_sensor
from .common import build_profile_numbers
from .common import build_profile_services
from .common import build_self_study_sensors
from .common import build_self_study_services
from .common import build_status_binary_sensors
from .common import build_wifi_binary_sensor
...
SERVICES = {
    ...
    "set_profile": {"name": "Set Profile", "endpoint": "set/prf/{profile}"},
...
SENSOR_DETAILS.update(build_profile_count_sensor(PROFILE_COUNT_ENDPOINT))
SENSOR_DETAILS.update(build_self_study_sensors())
SENSOR_DETAILS.update(build_network_sensors())

# The alarm endpoint also reports the history of the last alarms.
SENSOR_DETAILS["alarm_status"]["attributes"] = {"alarm_history": "getALM"}

# Profiles can be enabled/disabled and can be excluded from the leakage warning.
for _profile in range(1, 9):
    SENSOR_DETAILS[f"profile_{_profile}"].setdefault("attributes", {}).update(
        {
            "profile_enabled": f"getPA{_profile}",
            "leakage_warning_enabled": f"getPW{_profile}",
        }
    )

SERVICES.update(build_profile_services())
SERVICES.update(build_self_study_services())

NUMBERS = build_profile_numbers()

BINARY_SENSORS = {
    **build_alarm_binary_sensors(ALARM_CODES, ["A3", "A4", "A6", "A7", "A8"]),
    **build_status_binary_sensors(WARNING_CODES, NOTIFICATION_CODES),
    **build_microleakage_binary_sensor(),
    **build_wifi_binary_sensor(),
}
